...
function loop(isFirst) {//循环 函数   核心功能模块
    ...
    if(!Cross(fastLine,slowLine) && !isFirst) {//change
            return 0;
        }
...
//均线金叉函数
var fastLine = 4;//快线指标周期
var slowLine = 20;//慢线指标周期
var LineType = 1;//均线类型 0：MA   ，1:EMA
var avgLine = [TA.MA,TA.EMA][LineType];
function Cross(fastLine,slowLine){
    var records = null;
    while(true){
        records = exchange.GetRecords();
        if(records && records.length > (Math.max(fastLine, slowLine)+3)){
            break;
        }
        Sleep(Interval);
    }
